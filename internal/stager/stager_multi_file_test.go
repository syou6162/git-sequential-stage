...
// The actual implementation test is in hunk_info_test.go
...
func TestStageHunks_MultiFilePatches_Documentation(t *testing.T) {
	...
	t.Log("2. StageHunks extracts hunk N from the multi-file patch using go-gitdiff")
	t.Log("3. In execution phase, it gets a fresh diff for the specific file")
	t.Log("4. The patch IDs may differ between multi-file and single-file contexts")
	t.Log("")
